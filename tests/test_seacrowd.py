"""
Unit-tests to ensure tasks adhere to seacrowd schema.
"""
import argparse
import importlib
import sys
...
class TestDataLoader(unittest.TestCase):
    """
    Given a dataset script that has been implemented, check if it adheres to the `seacrowd` schema.
    ...
    def runTest(self):
        """
         ...
         (4) test_schema: confirms seacrowd keys present
        ...
        for schema in self.schemas_to_check:
            dataset_seacrowd = self.datasets_seacrowd[schema]
            ...
            for split_name, split in self.datasets_seacrowd[schema].items():
                print(split_name)
                print("=" * 10)
            ...
            if schema == "KB":
                with self.subTest("Check referenced ids"):
                    self.test_do_all_referenced_ids_exist(dataset_seacrowd)
                with self.subTest("Check passage offsets"):
                    self.test_passages_offsets(dataset_seacrowd)
                with self.subTest("Check entity offsets"):
                    self.test_entities_offsets(dataset_seacrowd)
                with self.subTest("Check events offsets"):
                    self.test_events_offsets(dataset_seacrowd)
                with self.subTest("Check coref offsets"):
                    self.test_coref_ids(dataset_seacrowd)

            elif schema == "QA":
                with self.subTest("Check multiple choice"):
                    self.test_multiple_choice(dataset_seacrowd)

    def setUp(self) -> None:
        """Load original and seacrowd schema views"""
        ...
        self.datasets_seacrowd = {}
        for schema in self.schemas_to_check:
            config_name = f"{self.SUBSET_ID}_seacrowd_{schema.lower()}"
            logger.info(f"Checking load_dataset with config name {config_name}")
            self.datasets_seacrowd[schema] = datasets.load_dataset(
                self.PATH,
                name=config_name,
                data_dir=self.DATA_DIR,
                use_auth_token=self.USE_AUTH_TOKEN,
    ...
    def get_feature_statistics(self, features: Features, schema: str) -> Dict:
        ...
        features in the schema present; only works for the seacrowd schema.
    ...
    def test_are_ids_globally_unique(self, dataset_seacrowd: DatasetDict):
        """
        Tests each example in a split has a unique ID.
        """
        logger.info("Checking global ID uniqueness")
        for split in dataset_seacrowd.values():
            ids_seen = set()
            for example in split:
    ...
    def test_do_all_referenced_ids_exist(self, dataset_seacrowd: DatasetDict):
        """
    ...
    def test_passages_offsets(self, dataset_seacrowd: DatasetDict):
        """
        ...
        for split in dataset_seacrowd:

            if "passages" in dataset_seacrowd[split].features:

                for example in dataset_seacrowd[split]:
    ...
    def test_entities_offsets(self, dataset_seacrowd: DatasetDict):
        """
        ...
        for split in dataset_seacrowd:
            ...
            if "entities" in dataset_seacrowd[split].features:
    ...
    def test_events_offsets(self, dataset_seacrowd: DatasetDict):
        """
        ...
        for split in dataset_seacrowd:
            ...
            if "events" in dataset_seacrowd[split].features:
    ...
    def test_coref_ids(self, dataset_seacrowd: DatasetDict):
        """
        ...
        for split in dataset_seacrowd:
            ...
            if "coreferences" in dataset_seacrowd[split].features:
    ...
    def test_multiple_choice(self, dataset_seacrowd: DatasetDict):
        """
    ...
    def test_schema(self, schema: str):
        """Search supported tasks within a dataset and verify seacrowd schema"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Unit tests for SEACrowd datasets. Args are passed to `datasets.load_dataset`")
    ...
    parser.add_argument(
        ...
        help="by default, seacrowd schemas will be discovered from _SUPPORTED_TASKS. use this to explicitly test only one schema.",
    )
    parser.add_argument(
        "--subset_id",
        default=None,
        required=False,
        help="by default, subset_id will be generated from path (e.g. if path=datasets/smsa.py then subset_id=smsa). the config name is then constructed as config_name=<subset_id>_seacrowd_<schema>. use this to explicitly set the subset_id for the config name you want to test.",
    ),
